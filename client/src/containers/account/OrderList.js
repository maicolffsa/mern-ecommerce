...
export const OrderList = () => {
  ...
  return (
    <OrderListWrapper>
      <div className="orders-header">Mis pedidos</div>
      {loading ? (
        <Loader />
      ) : error ? (
        <div className="tip">{error.msg}</div>
      ) : orders.length <= 0 ? (
        <div className="tip">Lista de pedidos vacía</div>
      ) : orders.length > 0 ? (
              ...
              <li key={order._id}>
                <Link to={"/order/" + order._id}>
                  <div className="single-order">
                    ...
                    ID del pedido: <strong>{order._id}</strong>
                    </div>
                    <div className="order-info">
                    Tiempo del pedido{" "}
                      <strong>
        ...
        <div className="tip">Algo salió mal.</div>
      )}
    </OrderListWrapper>
  );
};
